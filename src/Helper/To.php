...
final class To
{
	...
	public static function html( $value, string $title = null, string $color = null, bool $type = false ): string
	{
		$html  = '';
		$style
		       = 'border-spacing:0;border-collapse:collapse;font-family:Arial;width:100%;word-break:break-word;border-radius:3px;overflow:hidden;';
		$title = $title ?? ( $type
				? Str::type( $value )
				: null );
		if ( $title ) {
			...
			$html  .= '<thead style="background: #ddd;color: ' . $color .
			          ';"><tr><th colspan="2" style="padding:8px;font-size:14px;font-weight: normal;">' .
			          ucfirst( $title ) . '</th></tr></thead>';
		}
		if ( is_scalar( $value ) ) {
		...
		foreach ( $value as $k => $v ) {
			...
			$html .= '<tr style="font-size:12px;font-weight: normal;">' .
			         '<td style="background:#eee;padding:8px;border:1px solid #ddd;width:' . strlen( $k ) * 9 .
			         'px;">' . $k . '</td><td  style="padding:8px;border:1px solid #ddd;">' . $v . '</td></tr>';
		}
