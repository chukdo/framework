...
class Header
{
    ...
    public function getCacheControl(): Json
    {
        ...
        foreach ( explode( ', ', $this->getHeader( 'Cache-Control' ) ) as $value ) {
            ...
            } else if ( ( $age = Str::match( '/max-age=([0-9]+)/', $value ) ) !== false ) {
    ...
    public function setXFrameOptions( string $origin = null ): self
    {
        ...
        if ( isset( $_SERVER[ 'HTTP_REFERER' ] ) ) {
            ...
            foreach ( $origins as $origin ) {
                if ( $origin && substr( $uri->getHost(), -strlen( $origin ) ) == $origin ) {
                    $allow = true;
                    break;
                }
            }
    ...
    public function setAllowAllOrigin( string $origin = null, string $method = null, string $allow = null ): self
    {
        if ( isset( $_SERVER[ 'HTTP_ORIGIN' ] ) ) {
            ...
            $allow = $allow ?: 'Content-Type, Content-Range, Content-Disposition, Content-Description, '
                . 'Accept, Access-Control-Allow-Headers, Authorization, X-Requested-With';

            foreach ( $origins as $origin ) {
                if ( $origin && substr( $uri, -strlen( $origin ) ) == $origin ) {
                    $allow = true;
                    break;
                }
            }
    ...
    public function __call( $name, $params = [] )
    {
        ...
        $method = [
            ...
            'date' => 'Date', // Date RFC850
            'except' => 'Expect',
            'from' => 'From',
            'host' => 'Host',
            'ifmatch' => 'If-Match',
            'ifmodifiedsince' => 'If-Modified-Since', // Date RFC850
            'ifnonematch' => 'If-None-Match',
            'ifrange' => 'If-Range',
            'ifunmodifiedsince' => 'If-Unmodified-Since', // Date RFC850
            ...
            'etag' => 'ETag',
            'expires' => 'Expires', // Date RFC850
            'lastmodified' => 'Last-Modified', // Date RFC850
            'location' => 'Location',
